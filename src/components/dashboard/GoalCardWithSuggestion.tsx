...
export const GoalCardWithSuggestion: React.FC<GoalCardWithSuggestionProps> = ({
    ...
    const formatCurrency = (amount: number): string => {
        if (amount >= 1_000_000_000) {
            return currencySymbol + (amount / 1_000_000_000).toFixed(1).replace(/\.0$/, '') + 'B';
        }
        if (amount >= 1_000_000) {
            return currencySymbol + (amount / 1_000_000).toFixed(1).replace(/\.0$/, '') + 'M';
        }
        if (amount >= 1_000) {
            return currencySymbol + (amount / 1_000).toFixed(1).replace(/\.0$/, '') + 'K';
        }
            ...
            <View style={styles.amountRow}>
                ...
                {achieveInMonths !== undefined && (
                    <Text style={styles.achieveIn}>
                        Achieve in {achieveInMonths} {achieveInMonths === 1 ? 'month' : 'months'}
                    </Text>
                )}
            </View>
