...
export const AddGoalScreen: React.FC = () => {
  ...
  const [target, setTarget] = useState(
    suggestionTarget ? formatNumberInput(suggestionTarget.toString()) : '',
  );
  ...
  const handleSaveGoal = async () => {
    ...
    if (
      availableForNewGoals !== undefined &&
      contributionAmount > availableForNewGoals
    ) {
      Toast.show({
        type: 'error',
        text1: 'Excessive Contribution',
        text2: `Your contribution cannot exceed your available monthly savings (${currencySymbol}${formatCurrency(
        ...
        {!suggestionDescription &&
          ...
          contributionAmount > availableForNewGoals && (
            <View style={styles.mascotContainer}>
              <AnimatedMascot
                text={`You only have ${formatCurrency(
